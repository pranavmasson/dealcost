    ...
    const fetchReconditioning = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/api/reports/monthly?username=${username}&month=${month}&year=${year}`
        );
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        >
          <Box display="flex" alignItems="center" gap={2} mb={4}>
            <BuildIcon sx={{ fontSize: 40, color: '#4CAF50', opacity: 0.7 }} />
            <Box>
...
export default MonthlyReconditioning;
